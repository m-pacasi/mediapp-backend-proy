...
public class MenuServiceImpl implements IMenuService {
	...
	@Override
	public List<UsuarioRolDTO> listarUsuarioRol(String nombre) {
		List<UsuarioRolDTO> rol = new ArrayList<>();
		repo.listarUsuarioRol(nombre).forEach( x -> {
			UsuarioRolDTO r = new UsuarioRolDTO();
			r.setUsuario(String.valueOf(x[0]));
			r.setRol(String.valueOf(x[1]));
